...
module.exports = class Item {
    ...
    processJson(dataItem) {
        ...
        for (var jObj in dataItem) {
            switch (jObj.toLowerCase()) {
                ...
                case 'public':
                    var firstChar = dataItem[jObj].toLowerCase().substring(0, 1);
                    infoItem.visible = (firstChar === "y" || firstChar === "t") ? true : false;
                    break
                default:
                    infoItem.text = dataItem[jObj];
            }
        }
        return infoItem;
    }
    checkName(newName) {
        if (newName === "" | newName === "locations" || newName === "characters" || newName === "items" || newName === "all" ||
            this.infos.find(x => x.heading.toLowerCase() === newName.toLowerCase())) {
            return false;
        } else {
            return true;
        }
    ...
    view(msg, itemCodes, showPublic, gmFlag, channel, returnMsg) {
        ...
        if (returnMsg) {
            ...
            this.utility.sendMsg(channel, msgContent);
        }
    }
    ...
    renameInfo(msg, msgInfo2, msgInfo3) {
        ...
        if (this.infos.find(x => x.heading.toLowerCase() === msgInfo3.toLowerCase())) {
            this.utility.sendMsg(msg.channel, "ERROR: This item already has some info with that heading associated with it");
            return;
        }
    ...
    addInfo(msg, msgInfo2, msgInfo3, msgInfo4) {
        if (msgInfo2 === "") {
            this.utility.sendMsg(msg.channel, "ERROR: cannot add info with no header");
            return;
        }
        if (this.infos.find(x => x.heading.toLowerCase() === msgInfo2.toLowerCase())) {
            this.utility.sendMsg(msg.channel, "ERROR: This item already has some info with that heading associated with it");
            return;
        }
        var firstChar = msgInfo3.toLowerCase().substring(0, 1);
