...
module.exports = class GameManager {
    constructor() {
        ...
        this.locparObj = undefined;
        this.GMRoleID = undefined;
    ...
    msgHandler(msg, clientID) {
        if (msg.content.toLowerCase().startsWith(this.prefix.toLowerCase()) && (msg.guild === this.guild || this.initialised === false)) {
            ...
            switch (msgcontent.toLowerCase()) {
                case 'help':
                    this.getHelp(msg, msgInfo1);
                    break;
                case 'prefix':
                    this.setPrefix(msg, msgInfo1);
                    break;
                case 'setup':
                    this.setupCheck(msg, clientID);
                    break;
                case 'run':
                    this.run(msg);
                    break;
                ...
                case 'claim':
                    this.claimCharacter(msg, msgInfo1);
                    break;
                case 'release':
                    this.releaseCharacter(msg, msgInfo1);
                    break;
                case 'cast':
                case 'characters':
                case 'list':
                    this.viewChars(msg, false);
                    break
                case 'gm':
                    this.sendGMMsg(msg, msgInfo1 + " " + msgInfo2 + " " + msgInfo3 + " " + msgInfo4);
                    break;
                case 'msg':
                    this.sendCharMsg(msg, msgInfo1, msgInfo2 + " " + msgInfo3 + " " + msgInfo4);
                    break;
                case 'view':
                case 'inventory':
                case 'items':
                ...
                case 'give':
                case 'moveitem':
                    this.give(msg, msgInfo1, msgInfo2, msgInfo3);
                    break;
                case 'pay':
                    this.pay(msg, msgInfo1, msgInfo2, msgInfo3);
                    break;
                case 'steal':
                    this.steal(msg, msgInfo1, msgInfo2);
                    break;
                case 'move':
                case 'go':
                    this.move(msg, msgInfo1, msgInfo2);
                    break
                case 'leave':
                    this.leave(msg, msgInfo1);
                    break;
                case 'take':
                    this.take(msg, msgInfo1, msgInfo2);
                    break;
                case 'drop':
                ...
                case 'addchar':
                case 'newchar':
                    this.addChar(msg, msgInfo1);
                    break;
                case 'addloc':
                case 'newloc':
                    this.addLoc(msg, msgInfo1);
                    break;
                case 'additem':
                case 'newitem':
                    this.addItem(msg, msgInfo1, msgInfo2);
                    break;
                case 'addinfo':
                case 'newinfo':
                    this.addInfo(msg, msgInfo1, msgInfo2, msgInfo3, msgInfo4);
                    break;
                case 'delete':
                    this.removeCheck(msg, msgInfo1);
                    break;
                case 'deleteinfo':
                    this.deleteInfo(msg, msgInfo1, msgInfo2);
                    break;
                case 'rename':
                    this.renameID(msg, msgInfo1, msgInfo2);
                    break;
                case 'nickname':
                    this.renameNickname(msg, msgInfo1, msgInfo2 + (msgInfo3 === "", "", " " + msgInfo3) + (msgInfo4 === "", "", " " + msgInfo4), false);
                    break;
                case 'description':
                    this.renameNickname(msg, msgInfo1, msgInfo2 + (msgInfo3 === "", "", " " + msgInfo3) + (msgInfo4 === "", "", " " + msgInfo4), true);
                    break;
                case 'cash':
                case 'money':
                    this.changeCash(msg, msgInfo1, msgInfo2);
                    break;
                case 'prevent':
                    this.changePrevent(msg, msgInfo1, msgInfo2);
                    break;
                case 'renameinfo':
                    this.renameInfo(msg, msgInfo1, msgInfo2, msgInfo3);
                    break;
                case 'visibility':
                case 'visible':
                    this.editVisible(msg, msgInfo1, msgInfo2, msgInfo3);
                    break;
                case 'editinfo':
                case 'text':
                case 'detail':
                    this.editText(msg, msgInfo1, msgInfo2, msgInfo3 + (msgInfo4 === "", "", " " + msgInfo4));
                    break;
                case 'codes':
                    this.viewCodes(msg, msgInfo1);
                    break;
                case 'addcode':
                    this.addCode(msg, msgInfo1, msgInfo2);
                    break;
                case 'deletecode':
                    this.deleteCode(msg, msgInfo1, msgInfo2);
                    break;
            }
        }
    }
    ...
    checks(msg, checkInitialised, checkRunning, checkNotRunning, checkGM, allowPlayer) {
        if (this.initialised === false && checkInitialised) {
            this.utility.sendMsg(msg.channel, "ERROR: Bot is not initialised");
            return false;
        }
        if (this.running === false && checkRunning) {
            this.utility.sendMsg(msg.channel, "ERROR: Game is not currently running");
            return false;
        }
        if (this.running && checkNotRunning) {
            this.utility.sendMsg(msg.channel, "ERROR: Game is currently running");
            return false;
        }
        if (checkGM) {
            return this.checkGM(msg, allowPlayer);
        }
        return true;
    }
    checkGM(msg, allowPlayer) {
        ...
        if (msg.member === undefined) {
            this.utility.sendMsg(msg.channel, "ERROR: I can't work out which server you're on");
            return false;
        }
        if (msg.member.roles.cache.find(r => r.name.toLowerCase() === "gm") === undefined) {
            this.utility.sendMsg(msg.channel, allowPlayer ? "ERROR: I can't find your character" : "ERROR: You need a GM role to run this command");
            return false;
        }
        return true;
    }
    ...
    findObject(objectName, includeCharacters, includeLocations, includeItems, returnItem) {
        ...
        if (includeCharacters) {
            findObj = this.characters.find(x => x.nameID.toLowerCase() === objectName);
        }
        if (includeLocations && findObj === undefined) {
            findObj = this.locations.find(x => x.nameID.toLowerCase() === objectName);
        }
        if (findObj !== undefined || includeItems === false) {
            return findObj;
        }
        findObj = this.characters.find(x => x.items.find(y => y.nameID.toLowerCase() === objectName));
        if (findObj === undefined) {
            findObj = this.locations.find(x => x.items.find(y => y.nameID.toLowerCase() === objectName));
        }
        if (findObj !== undefined && returnItem) {
            findObj = findObj.items.find(x => x.nameID.toLowerCase() == objectName);
        }
        return findObj;
    }

    setupCheck(msg, clientID) {
        if (this.checks(msg, false, false, true, true, false) === false) {
            return;
        }
        if (this.initialised) {
    ...
    initCharacters(msg, fileData) {
        if (fileData.length > 0) {
            if (!fileData[0].hasOwnProperty('Name')) {
                this.utility.sendMsg(msg.channel, "ERROR: Cannot find 'Name' field for characters.csv");
            } else {
    ...
    async initItemsList(msg, fileData) {
        if (fileData.length > 0) {
            if (!fileData[0].hasOwnProperty('ItemName')) {
                this.utility.sendMsg(msg.channel, "ERROR: Cannot find ItemName field for items.csv");
            } else {
                for (var i = 0; i < fileData.length; i++) {
                    ...
                    if (findObj === undefined) {
                        ...
                        if (locObj === undefined) {
                            locObj = new Character({
                                name: "GM",
                                cash: 0
                            }, true, this.guild, this.locations.length, this.locparObj, this.clientID, this.GMRoleID, this.utility, msg);
    ...
    checkInitialisation(msg) {
        ...
        for (var i = 0; i < this.characters.length; i++) {
            ...
            for (var j = 0; j < codes.length; j++) {
                ...
                if (findObj === undefined) {
                    findObj = this.locations.find(x => x.items.find(y => y.infos.find(z => z.heading.toLowerCase() === code)));
                }
                if (findObj === undefined) {
        ...
        if (msgContent !== "") {
            this.utility.sendMsg(msg.channel, "WARNING: The following items have no associated information:" + msgContent);
        }
        if (msgContent2 != "") {
            this.utility.sendMsg(msg.channel, "WARNING: The following characters have information codes that do not match any item information:" + msgContent2);
        }
        this.initialised = true;
        this.utility.sendMsg(msg.channel, "Bot initialised. GM messages will be sent to this channel.");
    }
    run(msg) {
        if (this.checks(msg, true, false, true, true, false) === false) {
            return;
        }
        for (var i = 0; i < this.characters.length; i++) {
    ...
    stop(msg) {
        if (this.checks(msg, true, true, false, true, false) === false) {
            return;
        }
        for (var i = 0; i < this.characters.length; i++) {
    ...
    async sendGMMsg(msg, msgInfo) {
        if (this.checks(msg, true, false, false, false, true) === false) {
            return;
        }
        try {
            await this.GMChannel.send(this.utility.getAuthorName(msg) + ": " + msgInfo)
        } catch (error) {
            this.utility.sendMsg(msg.channel, "ERROR: Message did not send");
            return;
        };
    ...
    sendCharMsg(msg, msgInfo1, msgInfo2) {
        ...
        if (char === undefined) {
            this.utility.sendMsg(msg.channel, "Character cannot be found");
            return;
        }
    ...
    releaseCharacter(msg, msgInfo1) {
        ...
        if (char === true) {
            char = this.findObject(msgInfo1, true, false, false, false);
        }
        if (char === undefined) {
            this.utility.sendMsg(msg.channel, "Character cannot be found");
            return;
        }
        if (char.userID === undefined) {
            this.utility.sendMsg(msg.channel, "Character has not been claimed");
            return;
        }
    ...
    move(msg, msgInfo1, msgInfo2) {
        ...
        if (msgInfo1.toLowerCase() === "gm") {
            this.utility.sendMsg(msg.channel, "Error: The GM location is for GMs only; characters cannot move there");
            return;
        }
    ...
    view(msg, msgInfo1, msgInfo2, msgInfo3) {
        ...
        if (findObj === true) {
            ...
            if (findObj === undefined) {
                this.utility.sendMsg(msg.channel, "ERROR: Specified character, location, or item not found");
                return;
            }
            if (findObj instanceof Item) {
                findObj.view(msg, [msgInfo2], msgInfo2.toLowerCase() === "all" || msgInfo2 === "", msgInfo2.toLowerCase() === "all");
            } else {
                findObj.view(msg, msgInfo2, msgInfo3, true)
            }
        } else {
            findObj.view(msg, msgInfo1, msgInfo2, false);
        }
    }
    viewChars(msg, gmFlag) {
        if (this.characters.length === 0) {
            this.utility.sendMsg(msg.channel, "No characters found");
        } else {
        ...
        if (this.locations.length === 0) {
            this.utility.sendMsg(msg.channel, "No locations found");
        } else {
    ...
    show(msg, msgInfo1, msgInfo2, msgInfo3) {
        ...
        if (findObj === true) {
            ...
            if (findObj.nameID.toLowerCase() !== msgInfo1.toLowerCase()) {
                msgInfo3 = msgInfo1;
            }
            msgInfo1 = msgInfo2;
            msgInfo2 = msgInfo3;
        ...
        if (char2 === undefined) {
            this.utility.sendMsg(msg.channel, "Cannot find the character to show the item to");
            return;
        }
    ...
    give(msg, msgInfo1, msgInfo2, msgInfo3) {
        ...
        if (findObj === true) {
            ...
            char2 = this.findObject(msgInfo1, true, false, false, false);
        }
        if (char2 === undefined) {
            this.utility.sendMsg(msg.channel, "Cannot find the character to give the item to");
            return;
        }
    ...
    steal(msg, msgInfo1, msgInfo2) {
        ...
        if (findObj == true) {
            this.changeSteal(msg, msgInfo1, msgInfo2);
            return;
        }
        var char2 = this.findObject(msgInfo1, true, false, false, false);
        if (char2 === undefined) {
            this.utility.sendMsg(msg.channel, "Cannot find the character you're stealing from");
            return;
        }
    ...
    pay(msg, msgInfo1, msgInfo2, msgInfo3) {
        ...
        if (char2 === undefined) {
            this.utility.sendMsg(msg.channel, "Cannot find the character to pay the cash to");
            return;
        }
    ...
    findItem(msg, msgInfo1) {
        ...
        if (findObj === undefined) {
            findObj = this.findObject(msgInfo1, true, false, false, false);
            if (findObj === undefined) {
                this.utility.sendMsg(msg.channel, "ERROR: Specified item or character not found");
                return;
            }
            ...
            this.utility.sendMsg(msg.channel, findObj.nameID + " has item " + msgInfo1);
        }
    }
    addChar(msg, msgInfo) {
        if (this.checks(msg, true, false, false, true, false) === false) {
        ...
        if (this.nameExists(msgInfo)) {
            this.utility.sendMsg(msg.channel, "ERROR: Name already exists or is invalid");
            return;
        }
        this.characters.push(new Character({
            name: msgInfo,
            cash: 0
        }, false, this.guild, this.characters.length, this.charparObj, this.clientID, this.GMRoleID, this.utility, msg));
    ...
    addLoc(msg, msgInfo) {
        ...
        this.locations.push(new Character({
            name: msgInfo,
            cash: 0
        }, true, this.guild, this.locations.length, this.locparObj, this.clientID, this.GMRoleID, this.utility, msg));
    ...
    remove(msg, msgInfo1) {
        ...
        if (findObj.nameID.toLowerCase() !== msgInfo1.toLowerCase()) {
            findObj.removeItem(msg, msgInfo1);
            return;
        }
    ...
    renameID(msg, msgInfo1, msgInfo2) {
        ...
        if (findObj === undefined) {
            this.utility.sendMsg(msg.channel, "ERROR: Cannot find character, location, or item " + msgInfo1);
            return;
        }
        if (this.nameExists(msgInfo2)) {
            this.utility.sendMsg(msg.channel, "ERROR: Name already exists or is not valid");
            return;
        }
    ...
    renameNickname(msg, msgInfo1, msgInfo2, locationTrue) {
        ...
        if (findObj === undefined) {
            this.utility.sendMsg(msg.channel, "ERROR: Cannot find " + (locationTrue ? "location " : "character  ") + msgInfo1);
            return;
        }
    ...
    addItem(msg, msgInfo1, msgInfo2) {
        ...
        if (findObj === undefined) {
            this.utility.sendMsg(msg.channel, "ERROR: Cannot find character or location " + msgInfo1);
            return;
        }
        if (this.nameExists(msgInfo2)) {
            this.utility.sendMsg(msg.channel, "ERROR: Name already exists or is not valid");
    ...
    renameInfo(msg, msgInfo1, msgInfo2, msgInfo3) {
        ...
        if (findObj === undefined) {
            this.utility.sendMsg(msg.channel, "ERROR: Cannot find item " + msgInfo1);
            return;
        }
    ...
    addInfo(msg, msgInfo1, msgInfo2, msgInfo3, msgInfo4) {
        ...
        if (msgInfo2 == "") {
            this.utility.sendMsg(msg.channel, "Cannot add an item with no heading");
            return;
        }
        var findObj = this.findObject(msgInfo1, false, false, true, true);
        if (findObj === undefined) {
    ...
    deleteCode(msg, msgInfo1, msgInfo2) {
        if (!this.checks(msg, true, false, false, true, false)) {
            return;
        }
        var findObj = this.findObject(msgInfo1, true, false, false, false);
        if (findObj === undefined) {
    ...
    async saveAll(msg) {
        ...
        if (fs.existsSync('./csvs') === false) {
            try {
                await fs.mkdirSync('./csvs');
            } catch {
                this.utility.sendMsg(msg.channel, "ERROR: Folder /csvs/ does not exist and I can't create it, could you please make this folder and try again?");
                return;
            }
        }
    ...
    async saveCharacters(msg) {
        ...
        var itemCount = Math.max.apply(Math, this.characters.map(function(x) {
            return x.items.length;
        }));
        var codeCount = Math.max.apply(Math, this.characters.map(function(x) {
            return x.itemCodes.length;
        }));
        var charData = [];
        ...
        try {
            var csvWriter = createCsvWriter({
                path: './csvs/characters.csv',
                header: headerList
            });
            await csvWriter.writeRecords(charData).then(() => this.utility.sendMsg(msg.channel, "Characters.csv saved"));
        } catch {
            this.utility.sendMsg(msg.channel, "ERROR: Could not save to characters.csv, do you have it open?");
        }
    }
    async saveLocations(msg) {
        ...
        var itemCount = Math.max.apply(Math, this.locations.map(function(x) {
            return x.items.length;
        }));
        for (var i = 0; i < itemCount; i++) {
            headerList.push({
        ...
        try {
            ...
            this.utility.sendMsg(msg.channel, "ERROR: Could not save to locations.csv, do you have it open?");
        }
    }
    async saveItems(msg) {
        var headerList = [{
        ...
        try {
            ...
            this.utility.sendMsg(msg.channel, "ERROR: Could not save to items.csv, do you have it open?");
        }
    }
    nameExists(newName, ignoreOnce) {
        var firstExists = !ignoreOnce;
        var firstObj = undefined;
        newName = newName.toLowerCase();
        if (newName === "" || newName === "locations" || newName === "characters" || newName === "items" || newName === "all" || newName === "cash" || newName === "abilities" || newName === "ability") {
            return true;
        }
        ...
        if (firstObj) {
            if (firstExists) {
                ...
                firstExists = true;
                if (this.characters.find(x => x.nameID.toLowerCase() === newName && x !== firstObj)) {
                    return true;
                }
            }
        }
        ...
        if (firstObj) {
            if (firstExists) {
                ...
                firstExists = true;
                if (this.locations.find(x => x.nameID.toLowerCase() === newName && x !== firstObj)) {
                    return true;
                }
            }
        }
        ...
        if (firstObj) {
            if (firstExists) {
                ...
                firstExists = true;
                firstObj = firstObj.items.find(y => y.nameID.toLowerCase() === newName);
                if (this.characters.find(x => x.items.find(y => y.nameID.toLowerCase() === newName && y !== firstObj))) {
                    return true;
                }
            }
        }
        ...
        if (firstObj) {
            if (firstExists) {
                ...
                firstExists = true;
                firstObj = firstObj.items.find(y => y.nameID.toLowerCase() === newName);
                if (this.locations.find(x => x.items.find(y => y.nameID.toLowerCase() === newName && y !== firstObj))) {
                    return true;
                }
            }
        }
    ...
    findValidName(newName, count) {
        ...
        if (!this.nameExists(newText)) {
            return newText;
        } else {
            return this.findValidName(newName, count + 1);
        }
    }
}
