...
module.exports = class Character {
    constructor(jsonIn, _locTrue, _guild, _index, _parObj, _clientID, _gmID, _utility, msg) {
        ...
        this.gmID = _gmID;
        this.userOldName = undefined;
        this.userID = _locTrue ? [] : undefined;
        ...
        for (var jObj in jsonIn) {
            switch (jObj.toLowerCase()) {
                case 'name':
                    this.nameID = typeof(jsonIn[jObj]) === "string" ? jsonIn[jObj].replace(" ", "_") : jsonIn[jObj];
                    nameFound = true;
                    break;
                case 'nickname':
                case 'description':
                ...
                case 'steal':
                case 'theft':
                    this.stealCount = typeof(jsonIn[jObj]) === "string" ? Number(jsonIn[jObj].replace(/,/g, '')) : Number(jsonIn[jObj]);
                    if (Number.isNaN(this.stealCount) || this.stealCount < 0) {
                        this.stealCount = 0;
                        this.error = true;
                        stealError = true;
                    }
                    this.stealCount = Math.floor(this.stealCount);
                    break;
                case 'prevent':
                case 'stop':
                    this.preventCount = typeof(jsonIn[jObj]) === "string" ? Number(jsonIn[jObj].replace(/,/g, '')) : Number(jsonIn[jObj]);
                    if (Number.isNaN(this.preventCount) || this.preventCount < 0) {
                        this.preventCount = 0;
                        this.error = true;
                        preventError = true;
                    }
        ...
        if (nameFound === false) {
            this.index = _index;
            this.error = true;
            this.utility.sendMsg(msg.channel, "Character name not found for character " + _index);
        }
        if (cashError) {
            this.utility.sendMsg(msg.channel, "Could not read cash value for character " + (this.nameID === undefined ? _index : this.nameID));
        }
        if (stealError) {
            this.utility.sendMsg(msg.channel, "Could not read steal ability count for character " + (this.nameID === undefined ? _index : this.nameID));
        }
        if (preventError) {
            this.utility.sendMsg(msg.channel, "Could not read prevent theft ability count for character " + (this.nameID === undefined ? _index : this.nameID));
        }
        if (this.description === undefined) {
            this.description = this.nameID;
        }
    ...
    stop(msg) {
        this.running = false;
        if (!this.locTrue) {
            this.release(msg, undefined, undefined, true);
        }
        ...
        if (memberID !== undefined) {
            if (!this.locTrue) {
                if (this.running === false) {
                    this.utility.sendMsg(msg.channel, "Error: character claiming not currently running");
                    return;
                }
                if (this.error === true) {
                    this.utility.sendMsg(msg.channel, "There was an error building this character; check with GM");
                }
                if (this.userID !== undefined) {
                    this.utility.sendMsg(msg.channel, "Error: Character has already been claimed");
                    return;
                }
    ...
    async makeChannel(msg) {
        ...
        try {
            ...
            try {
                ...
                this.utility.sendMsg(msg.channel, "ERROR: Could not create new channel for " + this.nameID);
                this.channel = undefined;
                return;
            }
        }
        if (this.locTrue) {
            this.msgManager = new MSGManager(this.nameID, this.channel, this.utility, this.description, this.items, this.cash);
        }
    }
    ...
    leave(msg) {
        if (this.location === undefined) {
            this.utility.sendMsg(msg.channel, this.nameID + " is not in a location");
            return;
        }
        ...
        if (this.channel !== undefined) {
            if (this.channel.type !== 'dm') {
                try {
                    ...
                    this.utility.sendMsg(msg.channel, "ERROR: Could not delete the channel for " + this.nameID);
                }
            }
            this.msgManager = undefined;
        }
    ...
    async addPerm(msg, memberID) {
        if (this.channel === undefined) {
            if (!this._locationTrue) {
                this.channel = msg.member;
            }
    ...
    async removePerm(msg, memberID, isStop) {
        if (this.channel.type !== 'dm') {
            try {
                ...
                if (!isStop) {
                    this.utility.sendMsg(msg.channel, "ERROR: Could not remove user from channel for " + this.nameID + ". Check with GMs.");
                }
            }
        }
    }
    async release(msg, memberID, charNameID, isStop) {
        if (this.userID !== undefined) {
            if (this.locTrue) {
                ...
                try {
                    member = await this.guild.members.fetch(this.userID)
                } catch {}
                try {
                    await member.setNickname(this.userOldName);
                } catch {
                    this.utility.sendMsg(member, "Please change your nickname back");
                }
    ...
    view(msg, msgInfo1, msgInfo2, gmFlag, channel, itemCodes) {
        ...
        if (!itemCodes) {
            itemCodes = (msgInfo1 === "") ? (gmFlag ? [] : this.itemCodes) : ((msgInfo2 !== "") ? [msgInfo2] : this.itemCodes);
        }

        if (msgInfo1 === "" || (msgInfo1 === "all" && msgInfo2 === "") || msgInfo1 === "cash") {
            this.utility.sendMsg(channel, this.cash === 0 ? this.nameID + " has no cash" : this.nameID + " has " + this.cash.toLocaleString('en-GB') + " in cash");
        }
        if (msgInfo1 === "" || (msgInfo1 === "all" && msgInfo2 === "") || msgInfo1 === "ability" || msgInfo1 === "abilities") {
            if (this.stealCount !== 0) {
                this.utility.sendMsg(channel, this.nameID + " has " + this.stealCount + " uses of the stealing ability");
            }
            if (this.preventCount !== 0) {
                this.utility.sendMsg(channel, this.nameID + " has " + this.preventCount + " uses of the ability to resist stealing");
            }
            if ((msgInfo1 === "ability" || msgInfo1 === "abilities") && this.stealCount === 0 && this.preventCount === 0) {
                this.utility.sendMsg(channel, this.nameID + " has no ability to either steal or prevent theft");
            }
        }
        if (msgInfo1 === "cash" || msgInfo1 === "ability" || msgInfo1 === "abilities") {
            return;
        }
        if (this.items.length === 0 && this.examineItems.length === 0) {
            ...
            if (msgInfo1 !== "" && msgInfo1 !== "all") {
                ...
                if (findItem === undefined) {
                    findItem = this.examineItems.find(x => x.nameID.toLowerCase() === msgInfo1);
                }
                if (findItem === undefined) {
                    ...
                    if (!gmFlag && msgInfo2 !== "" && !itemCodes.find(x => x.toLowerCase() === msgInfo2)) {
                        this.utility.sendMsg(channel, "You do not have permission to view this code");
                        return;
                    }
    ...
    giveAllow(msg, char2, msgInfo2, stealFrom, stealMsgNow, channel) {
        ...
        if (itemIndex === -1) {
            if (stealFrom) {
                if (this.items.length === 0) {
                    this.utility.sendMsg(msg.channel, "This character has no items.");
                    return;
                }
        ...
        if (this.locTrue) {
            if (this.msgManager !== undefined) {
                this.msgManager.setItems(this.items, this.cash);
            } else {
                this.utility.sendMsg(msg.channel, "Location " + this.nameID + " has given item " + msgInfo2 + " to " + char2.nameID);
            }
        } else {
            if (stealFrom) {
                setTimeout((itemName) => {
                    if (this.channel) {
                        this.utility.sendMsg(this.channel, "<@" + this.userID + "> You have been stolen from!! You have lost item " + itemName)
                    }
                }, stealMsgNow ? 0 : 300000 + Math.floor(300000 * Math.random()), msgInfo2);
            } else {
                this.utility.sendMsg(channel, "You have given item " + msgInfo2 + " to " + char2.nameID);
            }
        }
    }
    async receive(msg, item) {
        ...
        if (this.channel !== undefined) {
            try {
                await this.channel.send((this.locTrue ? "A character has dropped item " : "<@" + this.userID + "> " + "You have received item ") + item.nameID);
            } catch {
                this.utility.sendMsg(msg.channel, "Error: could not send message to " + this.nameID + "'s channel - please let them know!");
            }
        }
    ...
    steal(msg, char2, msgInfo2) {
        if (this.stealCount <= 0) {
            this.utility.sendMsg(msg.channel, "You do not have any steal actions left.")
        }
        if (this.stealCount > 0) {
    ...
    async beShown(msg, item) {
        ...
        if (this.channel !== undefined) {
            try {
                await this.channel.send("<@" + this.userID + "> " + "You have been shown item " + item.nameID + ". You have 3 minutes to examine it");
            } catch {
                this.utility.sendMsg(msg.channel, "Error: could not send message to " + this.nameID + "'s channel - please let them know!");
            }
        }
        setTimeout(() => {
            var ind = this.examineItems.findIndex(x => x === item);
            this.examineItems.splice(ind, 1);
        }, 180000);
    }
    take(msg, msgInfo1) {
        if (this.location === undefined) {
            this.utility.sendMsg(msg.channel, "Error: You are not in a location");
            return;
        }
        if (msg.channel !== this.location.channel) {
            this.utility.sendMsg(msg.channel, "Error: You must type location commands into the correct channel (or ask a GM to use the 'give' command for you)");
            return;
        }
    ...
    examine(msg, msgInfo1, msgInfo2) {
        ...
        if (item === undefined) {
            item = this.examineItems.find(x => x.nameID.toLowerCase() === msgInfo1.toLowerCase());
        }
        if (item === undefined) {
            this.utility.sendMsg(msg.channel, "Error: Cannot find that item, have you typed the ID name correctly?");
            return;
    ...
    async sendInfo(msg, infoMessage) {
        try {
            await this.channel.send(infoMessage);
            this.utility.sendMsg(msg.channel, "Message sent");
        } catch {
            this.utility.sendMsg(msg.channel, "Error: Cannot send info to this character's private channel");
        }
    }
    pay(msg, payee, msgInfo2, channel) {
        ...
        if (Number.isNaN(msgInfo2)) {
            this.utility.sendMsg(msg.channel, "Error: cash amount not valid");
            return;
        }
        if (msgInfo2 < 0) {
            this.utility.sendMsg(msg.channel, "Error: you can't " + (this.locTrue ? "take " : " give ") + "a negative amount");
            return;
        }
        if (msgInfo2 > this.cash) {
            this.utility.sendMsg(msg.channel, "Error: you can't " + (this.locTrue ? "take " : " give ") + "more money than there is");
            return;
        }
        ...
        if (this.locTrue) {
            if (this.msgManager !== undefined) {
                ...
                this.utility.sendMsg(msg.channel, this.nameID + " has given " + msgInfo2.toLocaleString('en-GB') + " to " + payee.nameID);
            }
        }
        if (!this.locTrue) {
    ...
    async getPaid(msg, amount) {
        ...
        if (this.channel !== undefined) {
            try {
                await this.channel.send((this.locTrue ? "A character has dropped cash amount " : "<@" + this.userID + "> " + "You have been given cash amount ") + amount.toLocaleString('en-GB') + " in cash");
            } catch {
                this.utility.sendMsg(msg.channel, "Error: could not send message to " + this.nameID + "'s channel - please let them know!");
            }
        }
    ...
    async renameNickname(msg, msgInfo2) {
        ...
        if (!this.locTrue && this.userID !== undefined) {
            this.guild.members.fetch(this.userID).then(async member => {
                try {
    ...
    changeCash(msg, msgInfo2) {
        ...
        if (msgInfo2 < 0) {
            this.utility.sendMsg(msg.channel, "Error: cannot be a negative amount");
            return;
        }
        this.cash = msgInfo2;
    ...
    changeSteal(msg, msgInfo2) {
        msgInfo2 = Number(msgInfo2.replace(/,/g, ''));
        if (Number.isNaN(msgInfo2)) {
            this.utility.sendMsg(msg.channel, "Error: ability count not valid");
            return;
        }
        if (msgInfo2 < 0) {
            this.utility.sendMsg(msg.channel, "Error: cannot be a negative amount");
        ...
        if (!Number.isInteger(msgInfo2)) {
            this.utility.sendMsg(msg.channel, "Error: ability count must be a whole number");
            return;
        }
    ...
    deleteCode(msg, msgInfo2) {
        ...
        if (codeIndex === -1) {
            this.utility.sendMsg(msg.channel, "Error: this code is not listed for charcter " + this.nameID);
            return;
        }
    ...
    addCode(msg, msgInfo2) {
        var codeIndex = this.itemCodes.findIndex(x => x.toLowerCase() === msgInfo2.toLowerCase());
        if (codeIndex !== -1) {
            this.utility.sendMsg(msg.channel, "Error: this character can already view information with this code");
            return;
        }
    ...
    save(itemArrSize, codeArrSize) {
        ...
        if (this.locTrue) {
            ...
            charData = {
                name: this.nameID,
                nickname: this.description,
                steal: this.stealCount,
                prevent: this.preventCount,
                cash: this.cash
            };
        }
        if (!this.locTrue) {
